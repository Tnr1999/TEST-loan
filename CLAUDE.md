...
**สิทธิ์ที่ "เฉพาะ Owner เท่านั้น" (3 อย่าง):** ① **ยืนยันโอนเงิน** ("เปิด" สัญญาใหม่) · ② **คืนเครดิต** (ลูกค้าตายจ่ายเต็มยอด) · ③ **ลบลูกค้า** — role อื่นทำ 3 อย่างนี้ไม่ได้แม้แต่หัวหน้ากอง

- `owner` = **OWNER** — เห็น/จัดการทั้งระบบ · **ผู้เดียวที่ ยืนยันโอน + คืนเครดิต + ลบลูกค้า ได้**
- `head` = **หัวหน้ากอง** — เห็น+ทำได้**เหมือน Owner แต่เฉพาะในกองตัวเอง** (ผูกผ่าน `user_groups`): เพิ่มลูกค้า/แก้/รับเงิน/ปิด/เปิดยอดใหม่/เพิ่มยอด/ตาย + จัดการบ้านในกอง · **ยกเว้น ยืนยันโอน/คืนเครดิต/ลบลูกค้า ไม่ได้** (Owner เท่านั้น) · **ไม่มีค่าแรง/ไม่รับค่าคอม**
- `line` = **หัวหน้าสาย** — เห็น+จัดการเฉพาะ**สายบ้านตัวเอง** (ผูกผ่าน `user_branches`): เพิ่มลูกค้า/แก้/รับเงิน/ปิด/เปิดยอดใหม่/เพิ่มยอด/ตาย · ไม่จัดการโครงสร้างบ้าน · **ยืนยันโอน/คืนเครดิต/ลบลูกค้า ไม่ได้** · **รับค่าคอม 5% ของทั้งสาย** *(role เดิม `manager` ถือเป็นหัวหน้าสายด้วย)*
- `staff` = **พนักงาน** — เห็นเฉพาะบ้านตัวเอง (`user_branches`) · เพิ่มลูกค้า/รับเงิน/ปิด/เปิดยอดใหม่/เพิ่มยอด/กดตาย ได้ · **แก้ข้อมูลลูกค้าไม่ได้** · **ยืนยันโอน/คืนเครดิต/ลบลูกค้า ไม่ได้**

`canEdit()`=ทุก role (รับเงิน/ปิด/เพิ่มยอด) · `canAddCustomer()`=ทุก role (เพิ่มลูกค้า/เปิดยอดใหม่) · `canEditCustomerInfo()`=owner+หัวหน้ากอง+หัวหน้าสาย (พนักงานแก้ไม่ได้) · `canDisburse()`(ยืนยันโอน)/`canReturnCredit()`(คืนเครดิต)/`canDelete()`(ลบลูกค้า)=**owner เท่านั้น** · `canManageBranches()`=owner+หัวหน้ากอง · `canManageGroups()`/`canManageUsers()`=owner
...
- **กันลูกน้องโกง (กู้หลายที่/คนตายซ้ำ)** — ลิมิตเดิม `loanRuleError` (≤2 กอง · 1 บ้าน/กอง) + เสริมการตรวจตัวตนตอนเปิดสัญญาใหม่/เปิดยอดใหม่ใน `js/customers.js` `saveCustomer`:
  ...
  - **checksum เลขบัตร** (`validThaiId`, mod 11) → **บล็อก: กรอกผิดบันทึกไม่ได้** (ทั้งเปิดสัญญา+แก้ไข · โหมดแก้ไขบล็อกเฉพาะเมื่อมีการกรอกเลขบัตร) · **Owner ข้ามได้คนเดียว** (role อื่นบล็อกแข็ง · Owner ได้แค่ toast เตือนแล้วบันทึกต่อได้)
