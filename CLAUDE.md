...
- **ค่าแรง (แท็บแยก)** (`page-payout`, owner/head เห็น · รายวันตามวันที่ที่เลือก · ใช้วันร่วมกับ dashboard/ลูกค้าผ่าน `#dash-date-picker`) — ค่าแรง 20% (จาก `daily_records.wage`) − **คอม 5%** ของแต่ละคน → รวมเป็น "คอมของกอง" → จ่ายให้**หัวหน้ากอง** (รวมส่วนของหัวหน้าเองด้วย) · แยกตามกอง → รายคน (`recorded_by`) · การ์ดหัวหน้าโชว์ที่มาคอมรายคน · กติกา **1 กอง = หัวหน้า 1 คน** (หา head จาก `user_groups`+role) · กองไม่มีหัวหน้า = ไม่หักคอม · **เจ้าของระบบ (owner) ไม่คิดค่าแรง/คอม** · `renderPayoutPage`/`renderPayout`/`groupHeadUser` ใน `js/dashboard.js` *(ไม่ต้องรัน migration เพิ่ม — ใช้ข้อมูลเดิม)*
...
- **รหัสลูกค้า** = รหัสบ้าน + เลขลำดับในบ้าน (เช่น `AA01`) — `branches.code` (กรอกเองหน้าตั้งค่า) + `loans.cust_no` (ต่อคน · คงที่ถาวร · เปิดยอดใหม่ใช้เลขเดิม) · helper `custCode`/`nextCustNo` ใน `js/core.js` · แสดงแทน seq ในตาราง/การ์ด/รายละเอียด/หัวข้อรับเงิน · ยังไม่ตั้งรหัสบ้าน = fallback เป็น `#seq` *(ต้องรัน `supabase-migration-phase5-codes.sql` ก่อน)*
