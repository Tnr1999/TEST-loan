...
- `owner` = **OWNER** — เห็น/จัดการทั้งระบบ · **ผู้เดียวที่ "เปิดยอด" ได้** (เพิ่มลูกค้าใหม่/เปิดยอดใหม่/เพิ่มยอด/ยืนยันโอนเงิน)
- `head` = **หัวหน้ากอง** — เห็น+จัดการทุกอย่างเฉพาะ**ในกองตัวเอง** (ผูกผ่าน `user_groups`): แก้/รับเงิน/ปิด/คืนเครดิต/ลบ + จัดการบ้านในกอง · **เปิดยอดไม่ได้** (Owner เท่านั้น) · **ไม่มีค่าแรง/ไม่รับค่าคอม**
- `line` = **หัวหน้าสาย** — เห็น+จัดการทุกอย่างเฉพาะ**สายบ้านตัวเอง** (ผูกผ่าน `user_branches`): แก้/รับเงิน/ปิด/คืนเครดิต/ลบ · ไม่จัดการโครงสร้างบ้าน · **เปิดยอดไม่ได้** (Owner เท่านั้น) · **รับค่าคอม 5% ของทั้งสาย** *(role เดิม `manager` ถือเป็นหัวหน้าสายด้วย)*
- `staff` = **พนักงาน** — เห็นเฉพาะบ้านตัวเอง (`user_branches`) · **ดูอย่างเดียว + กดปุ่ม "ตาย" ได้เท่านั้น** — เพิ่ม/แก้ข้อมูลลูกค้า/รับเงิน/ปิด/ลบ/เปิดยอด ไม่ได้

`canEdit()`=owner+หัวหน้ากอง+หัวหน้าสาย (แก้/ลบ/รับเงิน/ปิด/คืนเครดิต) · `canDisburse()`=owner เท่านั้น ("เปิดยอด") · `canEditCustomerInfo()`=owner+หัวหน้ากอง+หัวหน้าสาย (พนักงานแก้ไม่ได้) · `canManageBranches()`=owner+หัวหน้ากอง · `canManageGroups()`/`canManageUsers()`/`canAddCustomer()`=owner
