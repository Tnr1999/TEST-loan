...
- `owner` = **OWNER** — เห็น/จัดการทั้งระบบ
- `head` = **หัวหน้ากอง** — เห็นทุกบ้านในกองตัวเอง (ผูกผ่าน `user_groups`) · จัดการบ้านในกองได้ · **ไม่รับค่าคอม** (คอมเข้าหัวหน้าสาย)
- `line` = **หัวหน้าสาย** — เห็นเฉพาะบ้านที่ดูแล (ผูกผ่าน `user_branches`) · แก้ไข/รับเงิน/ปิดสินเชื่อได้ · ไม่จัดการโครงสร้างบ้าน · **รับค่าคอม 5% ของทั้งสาย** *(role เดิม `manager` ถือเป็นหัวหน้าสายด้วย)*
- `staff` = **พนักงาน** — เห็นเฉพาะบ้านตัวเอง (`user_branches`) · เพิ่ม/แก้ข้อมูลลูกค้า + กดปุ่ม "ตาย" ได้ แต่รับเงิน/ปิด/ลบไม่ได้

`canEdit()`=owner+หัวหน้ากอง+หัวหน้าสาย · `canManageBranches()`=owner+หัวหน้ากอง · `canManageGroups()`/`canManageUsers()`=owner
