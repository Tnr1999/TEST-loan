...
function openPayment(custId,date){
  ...
  document.getElementById('modal-payment-body').innerHTML=
    ...
    '<div class="field"><label>ค่าปรับ (บาท)</label><input class="inp mono" id="pay-penalty" type="number" min="0" step="0.01" placeholder="0.00" value="'+(existing&&existing.penalty>0?existing.penalty:'')+'" oninput="updatePayCalc(\''+custId+'\')"/></div>'+
    '<div id="pay-calc"></div>'+
